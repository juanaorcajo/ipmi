...
for (int i = 0; i < nRed; i++) {
  ...
  float grosorBase = 0.5 + i * 0.08 * escala; 

  for (int j = 0; j < 2; j++) {
    float offset = j * 2;
    float x = posXStart + i * separacion + offset;

    for (int k = 0; k < 2; k++) {
      float yOffset = k * 1; 
      dibujarLinea(x,(height - altura) / 2 + yOffset,(height + altura) / 2 + yOffset,c,grosorBase);
    }
  }
}
  ...
  for (int i = 0; i < nCyan; i++) {
    ...
    float x = map(i, 0, nCyan - 1, xStart, xEnd); //orden
    float altura = map(i, 0, nCyan - 1, baseCyanMin, baseCyanMax); //altura suave
    color c = getCyanColor(i);
    float grosor = map(i, 0, nCyan - 1, 0.5f, 1.5f) * escala; //grosor progresivo 
   dibujarLinea(x, (height - altura) / 2, (height + altura) / 2, c, grosor);
  }
}
